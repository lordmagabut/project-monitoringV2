...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center flex-wrap">
        <h4 class="page-title m-0 text-center text-md-start w-100 w-md-auto mb-2 mb-md-0">
            {{ $proyek->pemberiKerja->nama_pemberi_kerja ?? '-' }} / {{ $proyek->nama_proyek }}
        </h4>
        <a href="{{ route('proyek.index') }}" class="btn btn-primary btn-sm d-none d-md-inline">Kembali</a>
        <a href="{{ route('proyek.index') }}" class="btn btn-primary w-100 d-block d-md-none">← Kembali</a>
    </div>
    ...
    <div class="card-body">
        ...
        <div class="card">
            {{-- Tab Header --}}
            <ul class="nav nav-tabs flex-wrap" id="lineTab" role="tablist">
            ...
            {{-- Tab Content --}}
            <div class="tab-content border border-top-0 p-2 p-md-3 mt-2" id="lineTabContent">
                ...
                <div class="tab-pane fade show active" id="detproyekContent" role="tabpanel">
                    <div class="table-responsive">
                        <table class="table table-sm">
                            ...
                            @endif
                            <tr><th style="width: 40%">PIC</th><td>{{ $proyek->pemberiKerja->pic }}</td></tr>
                                ...
                                <tbody>
                                    @foreach($headers as $h)
                                        ...
                                        <tr>
                                            ...
                                            <td>@if($schedule) Minggu ke-{{ $schedule->minggu_ke }} @endif</td>
                                            <td class="text-end">@if($schedule) {{ $schedule->durasi }} minggu @endif</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                ...
                <div class="tab-pane fade" id="progressContent" role="tabpanel">
                    <div class="mb-2">
                        <h5 class="mb-2">Data Progress Mingguan</h5>
                        <a href="{{ route('proyek.progress.create', $proyek->id) }}" class="btn btn-primary btn-sm w-100 w-md-auto">+ Input Progress</a>
                    </div>

                    {{-- Mobile card view --}}
                    <div class="d-block d-md-none">
                        @foreach ($progressSummary as $item)
                            <div class="card mb-2 p-2">
                                <strong>Minggu ke-{{ $item['minggu_ke'] }}</strong> <br>
                                Tanggal: {{ \Carbon\Carbon::parse($item['tanggal'])->format('d-m-Y') }} <br>
                                Sebelumnya: {{ number_format($item['progress_sebelumnya'], 2) }}%<br>
                                Pertumbuhan: {{ number_format($item['pertumbuhan'], 2) }}%<br>
                                Saat Ini: {{ number_format($item['progress_saat_ini'], 2) }}%<br>
                                Status: 
                                ...
                                @endif
                                <div class="mt-2">
                                    <a href="{{ route('proyek.progress.detail', [$proyek->id, $item['minggu_ke']]) }}" class="btn btn-sm btn-info w-100 mb-1">Lihat Detail</a>
                                    @if($item['status'] == 'draft')
                                        <form action="{{ route('proyek.progress.destroy', [$proyek->id, $item['minggu_ke']]) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus progress minggu ke-{{ $item['minggu_ke'] }}?')">
                                            @csrf @method('DELETE')
