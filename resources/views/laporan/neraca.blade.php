                ...
                <tbody>
                    ...
                    @foreach($data->groupBy('tipe') as $tipe => $rows)
                        ...
                        @foreach($rows as $row)
                            ...
                            @php
                                if ($tipe === 'Aset') $totalAset += $row['saldo'];
                                elseif ($tipe === 'Kewajiban') $totalKewajiban += $row['saldo'];
                                elseif ($tipe === 'Ekuitas') $totalEkuitas += $row['saldo'];
                            @endphp
                        @endforeach
                    @endforeach

                    <tr class="fw-bold table-light">
                        <td colspan="3" class="text-end">Total Aset</td>
